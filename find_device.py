#!/usr/bin/env python3
"""
Simple device discovery tool for SkyAcres ESP32 devices
"""
...
def test_ota_port(ip, port=3232):
    ...
    try:
        ...
        sock.close()
        if result == 0:
            print(f"✅ OTA port {port} is open on {ip}")
            return True
        else:
            print(f"❌ OTA port {port} is closed on {ip}")
            return False
    except Exception as e:
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python find_device.py <device_serial_number>")
        print("Example: python find_device.py 123456789123456789")
        sys.exit(1)
    ...
    if found_ips:
        print(f"\n🎉 Found {len(found_ips)} IPv4 addresses:")
        ...
        print(f"\n❌ Device {serial_number} not found on network")
        print("💡 Make sure:")
