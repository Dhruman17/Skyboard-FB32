...
// Function to fetch light intervals from Firestore
...
// Function to control GPIO 26 LED based on daily time intervals using digitalWrite
void controlGpio26LED() {
    ...
    // Convert the current time to just the time of day (hours, minutes, seconds)
    ...
    // Turn on the LED between the lightOnTime and lightOffTime
    if (currentTime_t >= lightOnTime && currentTime_t <= lightOffTime) {
        if (digitalRead(gpio26LEDPin) == LOW) {  // If LED is off
            digitalWrite(gpio26LEDPin, HIGH);   // Turn on the LED
            sendSystemNotification("GPIO 26 LED", "GPIO 26 LED turned ON");
            Serial.println("GPIO 26 LED turned ON");
        }
    } else {
        if (digitalRead(gpio26LEDPin) == HIGH) {  // If LED is on
            digitalWrite(gpio26LEDPin, LOW);    // Turn off the LED
            ...
            if (json.get(jsonData, "fields/unitState/booleanValue")) {
                ...
                if (unitStates[i] != newState) {
                    ...
                    if (unitStates[i]) {
                        ...
                        ledcWrite(i, 255); // Turn on the LED (full brightness)
...
void setup() {
    ...
    digitalWrite(gpio26LEDPin, LOW);  // Start with LED off
